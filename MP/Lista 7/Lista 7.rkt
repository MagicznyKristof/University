...
(define (eval-env e env)
  (cond [(const? e) e]
        [(is-and? e)
         (if (null? (cdr e))
             true
             (if (not (false? ((cadadr e))))
                 (eval (cons 'and (cdadr e)))
                 false
                 )
             )
         ]
        ...
        [(is-null? e) '()]
        [(is-cond? e)
         (cond [(= (length e) 1) (void)]
               [(and (= (length e) 2) (eq? (caadr e) 'else)) (eval (cadadr e))]
         [else (if (eval (caadr e))
                   (eval (cadadr e))
                   (eval (cons 'cond (cddr e))))])]
...
(define (new-list x)
  (if (empty? x)
      new-null
      (cons-cons (car x) (new-list (cdr x)))
  )
)
...
(define (op-bool-args e)
  (cdr e))

(define (op-bool-cons op args)
  (cons op args))
...
(define (new-if bool if-true if-false)
  (list 'if bool if-true if-false))
...
;; andy i ory nie działają chyba do końca, ale nie chciało mi się już poprawiać
