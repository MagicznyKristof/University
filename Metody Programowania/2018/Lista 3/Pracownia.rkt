...
(define (solve-ax+by=1 a b)
  (if (= (modulo a b) 1)
      (cons 1 (- (quotient a b)))
      (let ([p (solve-ax+by=1 b (modulo a b))])
        (cons (cdr p) (- (car p) (* (quotient a b) (cdr p))))
      )
  )
)
...
(define (RSA-unconvert-list intlist key)
  ...
  (let ((n (key-modulus key)))     
    (define (convert l prev)
      (if (null? l)
          null
          (let ([x (RSA-transform (car l) key)
                   ])
            (cons (modulo (+ x prev) n) (convert (cdr l) (car l))))))
    (convert intlist 0)))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  ;;; XXX: Write your code here
  (let ([crypt (RSA-encrypt message recipient-public-key)])
  (make-signed-message 
   (RSA-convert-list (cons (compress crypt) null) sender-private-key)
   crypt)
  )
)
...
(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (if (equal? (list (compress (message cyphertext))) (RSA-unconvert-list (signature cyphertext) sender-public-key))
      ; (car (RSA-unconvert-list (signature cyphertext) sender-public-key))))
      (RSA-decrypt (message cyphertext) recipient-private-key)
      (error "Sygnalizuje problem")
  )
)
