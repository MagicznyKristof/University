...
#|
UWAGA
Lambdy-mul nie działają, może je kiedyś poprawię
A, no i zawieszają program, więc przed uruchomieniem należy poprawić
;|#
...
#|
(define (lambda-list-cons vars e)
  (list 'lambda vars e))

(define (lambda-list-vars e)
  (cadr e))
;|#
...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(lambda-mul? e)
         (closure-cons (list (lambda-mul-var e)) (lambda-mul-expr e) env)]
        [(lambda-rec? e)
...
(eval '(let (append (lambda-rec (append-inside xs1 xs2) 
                                       (if (null? xs1)
                                           xs2
                                           (cons (car xs1)
                                                      (append-inside (cdr xs1) xs2)))))
         (append (cons 1 (cons 2 null)) (cons 3 (cons 4 null)))))

(eval '(let (map (lambda-rec (map-inside proc xs)
                             (if (null? xs)
                                 null
                                 (cons (proc (car xs)) (map-inside proc (cdr xs))))))
         (map (lambda (x) (+ 1 x)) (cons 1 (cons 2 (cons 3 null))))))

(eval '(let (reverse (lambda-rec (reverse-inside xs acc)
                                 (if (null?  xs)
                                     acc
                                     (reverse-inside (cdr xs) (cons (car xs) acc)))))
         (reverse (cons 2 (cons 3 (cons 4 null))) null)))

(define (lappend xs xl)
  (if (null? xs)
      xl
      (cons (car xs) (lappend (cdr xs) xl))))

(define (lmap proc xl)
  (lcons (proc (lhead xl)) (lambda () (lmap proc (ltail xl)))))
  

(define (my-take n l)
  (if (procedure? (cdr l))
      (take n l)
      (cons (car l) (my-take (- n 1) (cdr l)))))


(define (lreverse xl)
  (define (revers-iter xl acc)
    (revers-iter (ltail xl) (cons (lhead xl) acc))
    )
  (revers-iter xl null)
  )

(take 10 (lreverse nats))

(take 10 (lmap (lambda (x) (+ 1 x)) nats)) 