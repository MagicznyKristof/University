...
let-exp x e1 e2
   let v (eval e1 env)
       (eval e2 (add x v env))    <- część evala
...
|
v
x to x, 5 to e1, lambda to e2, v to 5
...
(define reverse-rec
  (letrec-expr 'rev
               (lambda-expr 'pair (let-expr 'xs (car-expr (variable 'pair))
                                            (let-expr 'ys (cdr-expr (variable 'pair))
                                                      (if-expr (null?-expr (variable 'xs))
                                                               (variable 'ys)
                                                               (app-expr (variable 'rev)
                                                                         (cons-expr (cdr-expr (variable 'xs))
                                                                                    (cons-expr (car-expr (variable 'xs))
                                                                                               (variable 'ys))))))))
               (app-expr (variable 'rev) (cons-expr (cons-expr (const 5) (cons-expr (const 6) (null-expr))) (null-expr)))))
