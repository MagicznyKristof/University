  ...
  if (!index(argv[0], '/') && path) {
    ...
    while (path) {
      size_t curr_path_len =
        strcspn(path, ":"); // length of path until ':' appears
      char *curr_path =
        strndup(path, curr_path_len); // copy the relevant fragment
      strapp(&curr_path, "/");
      strapp(&curr_path, argv[0]);            // add program name to string
      (void)execve(curr_path, argv, environ); // call execve
      path += curr_path_len + 1;              // increase path to after ':'
      free(curr_path);                        // free the pointer at curr_path
    }
  } else {
