...
int main(){
    ...
    lua_pushstring(L, "Spider-Man, Spider-Man, Does whatever a spider can");
    lua_setglobal(L, "Spiderman");
    ...
    if (luaL_loadfile(L, "spiderman.lua")  || lua_pcall(L, 0, 0, 0))
        error(L, "cannot load Spiderman file: %s\n", lua_tostring(L, -1));
